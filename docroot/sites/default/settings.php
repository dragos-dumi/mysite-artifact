...
// If env variables don't exists, we assume that it's a call outside docker
// and so we route database to docker container through local host and MYSQL_DEBUG_PORT
if (empty($_ENV['MYSQL_DATABASE'])) {
  $vars = parse_ini_file(DRUPAL_ROOT . '/../.env');
  $_ENV['MYSQL_DATABASE'] = $vars['MYSQL_DATABASE'];
  $_ENV['MYSQL_USER'] = $vars['MYSQL_USER'];
  $_ENV['MYSQL_PASSWORD'] = $vars['MYSQL_PASSWORD'];
  $_ENV['MYSQL_HOST'] = '127.0.0.1';
  $_ENV['MYSQL_PORT'] = $vars['MYSQL_DEBUG_PORT'];
...
$config['shield.settings']['user'] = 'visitor';
$config['shield.settings']['print'] = '';
$config['shield.settings']['allow_cli'] = TRUE;
// Overwrite password on each environment where shield is needed.
$config['shield.settings']['pass'] = !empty($_ENV['SHIELD_PASSWORD']) ? $_ENV['SHIELD_PASSWORD'] : 'password';

$settings['environment'] = !empty($_ENV['ENVIRONMENT']) ? $_ENV['ENVIRONMENT'] : 'dev';

$settings['file_public_path'] = !empty($_ENV['PUBLIC_FILES']) ? $_ENV['PUBLIC_FILES'] : 'sites/default/demo-files';

$settings['file_private_path'] = !empty($_ENV['PRIVATE_FILES']) ? $dir . '/' . $_ENV['PRIVATE_FILES'] : $dir . '/private-files';
